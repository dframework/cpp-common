...
build_cp_lib()
{
  ...
  mkdir -p ios${IOS_SDKVERSION}-${arch}/lib

  local org_dir="${CURRENTPATH}/bin/${lplat}${IOS_SDKVERSION}-${orgarch}.sdk"
  local dst_dir="${D_HOME}/ios${IOS_SDKVERSION}-${arch}"

  local org="${CURRENTPATH}/bin/${lplat}${IOS_SDKVERSION}-${orgarch}.sdk/lib/${2}"
  local dst="${D_HOME}/ios${IOS_SDKVERSION}-${arch}/lib/"

  cp ${org_dir}/lib/${2} ${dst_dir}/lib/

  if [ "libssl.a" = "${2}" ]; then
      cp -R ${org_dir}/include ${dst_dir}/
  fi
}

build_cp_libs()
{
  build_cp_lib "armv7" "${1}"
  build_cp_lib "armv7s" "${1}"
  build_cp_lib "arm64" "${1}"
  build_cp_lib "i386" "${1}"
  build_cp_lib "x86_64" "${1}"
}

build_last(){
  cd $D_HOME
  build_cp_libs "libssl.a"
  build_cp_libs "libcrypto.a"
}
